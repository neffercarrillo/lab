...
class BlogPostDAO:
    ...
    def increment_likes(self, permalink, comment_ordinal):
        ...
        print("i am trying to increment likes...")
        
        if post is not None:
            comment = post['comments'][comment_ordinal]
            print(comment)
            if 'num_likes' not in comment:
                comment['num_likes'] = int(1)
            else:
                comment['num_likes'] = int(comment['num_likes']) + int(1)

            # update post in the database
            self.posts.update_one({'permalink':permalink},{'$set':{'comments':post['comments']}})
